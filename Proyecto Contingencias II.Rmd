...
# tabla_prob: prob_hombres o prob_mujeres
modelo_estoc <- function(simulaciones, tabla_prob, ingresos) {
  ...
  for (simulacion in 1:simulaciones) {
    ...
    gastos_estoc <-
      ifelse(escenario == 0, 0, ifelse(escenario == 1, 1430000, ifelse(
        escenario == 2, 2119000, ifelse(
          escenario == 3,
          10089000,
    ...
    # Si la persona está muerta, solo se hace un pago
    for (j in 1:47) {
      dgn <- which(diag(gastos_estoc[j:nrow(gastos_estoc),]) == 3100000)[1]
      
      if (!is.na(dgn)) {
        for (i in 1:(97 - j)) {
          gastos_estoc[i + j, i + 1] <- ifelse(escenario[i + j - 1, i] == 5, 0, gastos_estoc[i + j, i + 1])
        }
      }
    }
...
b <-
  ifelse(a == 0, 0, ifelse(a == 1, 1430000, ifelse(
    a == 2, 2119000, ifelse(a == 3,
                            10089000,
                            ifelse(
                              a == 4, 14418000, ifelse(a == 5, 3100000, 0)
                            ))
  )))

for (j in 1:47) {
  dgn <- which(diag(b[j:nrow(b), ]) == 3100000)[1]
  
  if (!is.na(dgn)) {
    for (i in 1:(97 - j)) {
      b[i + j, i + 1] <- ifelse(a[i + j - 1, i] == 5, 0, b[i + j, i + 1])
    }
  }
}
