...
Una de las variables que más se cree que afecta el análisis son las probabilidades de transición, debido a que se tomó un supuesto muy simplificado para estas, por lo cual, se modifican estas, reduciendo en $10\%$ las probabilidades de estar sano y repartiéndolas en las probabilidades restantes.
...
for (i in 1:6) {
  ...
  modificacion_hombres <- ((prob_hombres_mod[[i]][, 1] * 0.1) / 5)
  modificacion_mujeres <- ((prob_mujeres_mod[[i]][, 1] * 0.1) / 5)
  
  # Se modifica la primer columna
  prob_hombres_mod[[i]][, 1] <- prob_hombres_mod[[i]][, 1] * 0.9
  prob_mujeres_mod[[i]][, 1] <- prob_mujeres_mod[[i]][, 1] * 0.9
