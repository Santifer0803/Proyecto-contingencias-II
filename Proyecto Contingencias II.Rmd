...
modelo_estoc <- function(simulaciones, tabla_prob) {
  ...
  for (simulacion in 1:simulaciones) {
    ...
    escenario[edades, 1] <- rep(0, 47)
    
    estados <- escenario[edades, 1]
    
    matrices_random <- lapply(edades, function(x) {
      matrix(runif(97 * 6), nrow = 97, ncol = 6)
    })
    
    intervalos <- lapply(edades, function(x) {
      unname(c(0, cumsum(c(tabla_prob[[1]][edades[x],]))))
    })
    
    for (i in edades) {
      for (j in 1:(length(intervalos[[i]]) - 1)) {
        if (!(between(matrices_random[[i]][i, j], intervalos[[i]][j], intervalos[[i]][j + 1]))) {
    ...
    edades <- c(edades, rep(47, 3))
    
    for (k in 1:50) {
      escenario[edades[k] + k, 1 + k] <- estados[min(k, 47)]
      
      if (estados[min(k, 47)] != 5) {
        intervalos <- unique(unname(c(0, cumsum(
          c(tabla_prob[[estados[min(k, 47)] + 1]][edades[k] + k, ])
        ))))
        
        estados_comp <- rep(0, 47)
        
        for (i in 1:47) {
          for (j in 1:(length(intervalos[[i]]) - 1)) {
            if (!(between(matrices_random[[i]][i + k, j], intervalos[[i]][j], intervalos[[i]][j + 1]))) {
              estados_comp[i] <- estados_comp[i] + 1
        ...
        if (estados_comp[min(k, 47)] < estados[min(k, 47)]) {
          estados[min(k, 47)] <- estados[min(k, 47)] - 1
          
        } else {
          estados[min(k, 47)] <- estados_comp[min(k, 47)]
...
escenario <- matrix(NA, nrow = 97, ncol = 98)
    ...
    intervalos <- lapply(edades, function(x) {
      unname(c(0, cumsum(c(prob_hombres[[1]][edades[x],]))))
    })
