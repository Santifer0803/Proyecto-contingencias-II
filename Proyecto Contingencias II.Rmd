...
Con esto, se procede a determinar las primas brutas anuales por edad para cada sexo.

```{r primas_brutas}
# Tasa equivalente que se usará en los cálculos
j <- 0.0097

# Montos de los beneficios sin gastos
S <- 3000000
c1 <- 1380000
c2 <- 2069000
c3 <- 10039000
...
# Vectores con las primas brutas
primas_brutash <- c()
primas_brutasm <- c()
...
for (x in 18:64) {
  ...
  prima <- (S * Ax_05 + c1 * ax01 + c2 * ax02 + c3 * ax03 + c4 * ax04) / (ax00)
  # Guardamos la prima para alguien de edad x en el vector de primas
  primas_brutash <- c(primas_brutash, prima)
  ...
  prima <- (S * Ax_05 + c1 * ax01 + c2 * ax02 + c3 * ax03 + c4 * ax04) / (ax00)
  # Guardamos la prima para alguien de edad x en el vector de primas
  primas_brutasm <- c(primas_brutasm, prima)
}
```

Ahora se determinarán las primas netas anuales por edad para cada sexo.

```{r primas_netas}
# Montos de los beneficios con gastos
S <- S + 100000
c1 <- c1 + 50000
c2 <- c2 + 50000
c3 <- c3 + 50000
...
for (x in 18:64) {
  ...
  prima <- (S * Ax_05 + c1 * ax01 + c2 * ax02 + c3 * ax03 + c4 * ax04) / (0.9 *
                                                                           ax00 - 0.1)
  # Guardamos la prima para alguien de edad x en el vector de primas
  primas_netash <- c(primas_netash, prima)
  ...
  prima <- (S * Ax_05 + c1 * ax01 + c2 * ax02 + c3 * ax03 + c4 * ax04) / (0.9 *
                                                                           ax00 - 0.1)
  # Guardamos la prima para alguien de edad x en el vector de primas
  primas_netasm <- c(primas_netasm, prima)
}
```

Seguidamente generamos las tablas en formato latex de las primas brutas y netas.

```{r tablas_primas}
df_primas_netas <- data.frame(primas_netash, primas_netasm)
df_primas_brutas <- data.frame(primas_brutash, primas_brutasm)

colnames(df_primas_netas) <- c("Hombre", "Mujer")
colnames(df_primas_brutas) <- c("Hombre", "Mujer")

rownames(df_primas_netas) <- 18:64
rownames(df_primas_brutas) <- 18:64

tabla_primas_netas <- xtable(df_primas_netas)
tabla_primas_brutas <- xtable(df_primas_brutas)

print(tabla_primas_netas, type = "latex")
print(tabla_primas_brutas, type = "latex")
```

Por último, realizamos los gráficos para visualizar ambos tipos de primas.

```{r graficos_primas}
# Gráficos primas netas
ggplot(df_primas_netas[1:38, ], aes(x = 18:55)) +
  geom_line(aes(y = Hombre / 1000000, color = "H"), linewidth = 1) +
  geom_line(aes(y = Mujer / 1000000, color = "M"), linewidth = 1) +
  labs(x = "Edad", y = "Prima comercial", color = "Leyenda") +
  scale_color_manual(
    name = "Sexo", 
    values = c("H" = "magenta3", "M" = "salmon2"),
    labels = c("H" = "Hombre", "M" = "Mujer")
  ) +
  theme_minimal()

ggplot(df_primas_netas[39:47, ], aes(x = 56:64)) +
  geom_line(aes(y = Hombre / 1000000, color = "H"), linewidth = 1) +
  geom_line(aes(y = Mujer / 1000000, color = "M"), linewidth = 1) +
  labs(x = "Edad", y = "Prima comercial", color = "Leyenda") +
  scale_color_manual(
...
# Gráficos primas brutas
ggplot(df_primas_brutas[1:38, ], aes(x = 18:55)) +
  geom_line(aes(y = Hombre / 1000000, color = "H"), linewidth = 1) +
  geom_line(aes(y = Mujer / 1000000, color = "M"), linewidth = 1) +
  labs(x = "Edad", y = "Prima de riesgo", color = "Leyenda") +
  scale_color_manual(
    name = "Sexo", 
    values = c("H" = "gold2", "M" = "royalblue"),
    labels = c("H" = "Hombre", "M" = "Mujer")
  ) +
  theme_minimal()

ggplot(df_primas_brutas[39:47, ], aes(x = 56:64)) +
  geom_line(aes(y = Hombre / 1000000, color = "H"), linewidth = 1) +
  geom_line(aes(y = Mujer / 1000000, color = "M"), linewidth = 1) +
  labs(x = "Edad", y = "Prima de riesgo", color = "Leyenda") +
  scale_color_manual(
...
# Se modifican los nombres de los sexos
cartera_seguros <- cartera_seguros %>%
  mutate(Sexo = recode(Sexo, "F" = "Mujer", "M" = "Hombre"))

# Cantidad de personas en la cartera dividida por sexo
cartera_seguros %>%
  ...
  scale_fill_manual(values = c("Hombre" = "darkorchid", "Mujer" = "darkorange"))

# Cantidad de personas en la cartera por edad dividida por sexo
cartera_seguros %>%
...
En esta sección se implementarán dos modelos estocásticos. El primero simula la cantidad de personas en los diferentes estados al final de cada año a partir del 2024. El segundo obtendrá los montos esperados de ingresos y egresos para cada uno de los estados. Ambos modelos se realizarán para cada sexo.
Para llevar a cabo lo anterior, se desarrollará una función que devuelva una lista que tendrá  otras tres listas: una con la proyección de personas en cada estado para cada año y otras dos con la proyección de gastos e ingresos para cada uno de los estados.
...
hombres_cartera <- table(poblacion_cartera$Edades[poblacion_cartera$Sexo == "Hombre"])
mujeres_cartera <- table(poblacion_cartera$Edades[poblacion_cartera$Sexo == "Mujer"])
...
esperanzas_personas_h <- Reduce("+", simulaciones_h[[1]]) / n_sim
esperanzas_personas_m <- Reduce("+", simulaciones_m[[1]]) / n_sim

# Esperanzas por estado de gastos
esperanzas_gastos_h <- Reduce("+", simulaciones_h[[2]]) / n_sim
esperanzas_gastos_m <- Reduce("+", simulaciones_m[[2]]) / n_sim

# Esperanzas por estado de ingresos
esperanzas_ingresos_h <- Reduce("+", simulaciones_h[[3]]) / n_sim
esperanzas_ingresos_m <- Reduce("+", simulaciones_m[[3]]) / n_sim
```

## Análisis de resultados

Finalmente, se realizan unos gráficos descriptivos de los modelos estocásticos correspondientes.

```{r graficos_finales}
# Se transpone el dataframe de personas hombres
df_personas <- as.data.frame(t(esperanzas_personas_h))

# Se agregan los años al dataframe
df_personas$anno <- 2024:2121

# Se cambian los nombres de las columnas
colnames(df_personas) <-
  c("est_0", "est_1", "est_2", "est_3", "est_4", "est_5", "annos")

# Gráfico de personas, estados 0 y 1
ggplot(df_personas, aes(x = annos)) +
  geom_line(aes(y = est_0, color = "0"), linewidth = 1) +
  geom_line(aes(y = est_1, color = "1"), linewidth = 1) +
  labs(x = "Año", y = "Cantidad de personas") +
  scale_colour_manual(name = "Estado",
                      values = c("0" = "lightpink2",
                                 "1" = "salmon2")) +
  cowplot::theme_cowplot()

# Gráfico de personas, estados del 2 al 5
ggplot(df_personas, aes(x = annos)) +
  geom_line(aes(y = est_2, color = "2"), linewidth = 1) +
  geom_line(aes(y = est_3, color = "3"), linewidth = 1) +
  geom_line(aes(y = est_4, color = "4"), linewidth = 1) +
  geom_line(aes(y = est_5, color = "5"), linewidth = 1) +
  labs(x = "Año", y = "Cantidad de personas") +
  scale_colour_manual(
    name = "Estado",
    values = c(
...
# Se transpone el dataframe de personas mujeres
df_personas <- as.data.frame(t(esperanzas_personas_m))
...
# Dataframe de comparación de personas por sexo
df_comparacion <- esperanzas_personas_h - esperanzas_personas_m

# Se transpone el dataframe de personas mujeres
df_comparacion <- as.data.frame(t(df_comparacion))

# Se agregan los años al dataframe
df_comparacion$anno <- 2024:2121

# Se cambian los nombres de las columnas
colnames(df_comparacion) <-
  c("est_0", "est_1", "est_2", "est_3", "est_4", "est_5", "annos")

# Gráfico de personas, estados del 0 al 1
ggplot(df_comparacion, aes(x = annos)) +
  geom_line(aes(y = est_0, color = "0"), linewidth = 1) +
  geom_line(aes(y = est_1, color = "1"), linewidth = 1) +
  geom_line(aes(y = est_2, color = "2"), linewidth = 1) +
  geom_line(aes(y = est_3, color = "3"), linewidth = 1) +
  ...
  labs(x = "Año", y = "Diferencia de personas") +
  scale_colour_manual(
    name = "Estado",
    values = c(
      "0" = "lightpink2",
...
# Dataframe para los gastos e ingresos
df_flujos <-
  data.frame(
    sexo = c("Hombre", "Mujer"),
    gastos = c(sum(esperanzas_gastos_h), sum(esperanzas_gastos_m)),
    ingresos = c(sum(esperanzas_ingresos_h), sum(esperanzas_ingresos_m))
  )

# Dataframe para el flujo neto
df_flujos <- df_flujos %>%
  mutate(neto = ingresos - gastos)

# Se ponen los flujos en formato largo
df_flujos <- df_flujos %>%
  pivot_longer(
    cols = c(gastos, ingresos, neto),
    names_to = "Categoria",
    values_to = "valor"
  )

# Gráfico de gastos, ingresos y flujo neto
ggplot(df_flujos, aes(
  x = sexo,
  y = (valor / 1000000000),
  fill = Categoria
)) +
  ...
  scale_fill_manual(
    ...
    labels = c("gastos" = "Gastos", "ingresos" = "Ingresos", "neto" = "Neto")
  ) +
  theme_minimal()
```

```{r tablas_modelos_estocasticos}
tabla_flujos <- xtable(df_flujos)
print(tabla_flujos, type = "latex")
```

# Análisis de sensibilidad

A continuación se realizarán dos análisis de sensibilidad que ayudarán a construir un análisis más completo del llevado a cabo.

## Tamaño de la cartera

Debido a que las primas del producto son muy elevadas para edades avanzadas, para este análisis de sensibilidad, y por realismo, se asumirá que solo tomarán la póliza las personas con edad de 18 a 50 años de la cartera construida. 

```{r sensibilidad_cartera}
# Definimos la semilla para reproducibilidad
set.seed(2024)
...
# Guardamos la cantidad de personas de la cartera con edad de 18 a 50
# Por lo que la cantidad de personas de edad 51 a 64 es de 0
hombres_sen <- c(hombres_cartera[1:33], rep(0, length(hombres_cartera) - 33))
mujeres_sen <- c(mujeres_cartera[1:33], rep(0, length(mujeres_cartera) - 33))

# Determinamos las matrices asociadas a las simulaciones determinadas, tanto para hombres y mujeres
sim_hsen <- modelo_estoc(n_sim, prob_hombres, hombres_sen, ingresos_h)
sim_msen <- modelo_estoc(n_sim, prob_mujeres, mujeres_sen, ingresos_m)

# Matrices con las esperanzas por estado y para cada año, para cada sexo
esp_per_hsen <- Reduce("+", sim_hsen[[1]]) / n_sim
esp_per_msen <- Reduce("+", sim_msen[[1]]) / n_sim

# Esperanzas por estado de gastos
esp_gast_hsen <- Reduce("+", sim_hsen[[2]]) / n_sim
esp_gast_msen <- Reduce("+", sim_msen[[2]]) / n_sim

# Esperanzas por estado de ingresos
esp_ing_hsen <- Reduce("+", sim_hsen[[3]]) / n_sim
esp_ing_msen <- Reduce("+", sim_msen[[3]]) / n_sim
```

Graficamos los resultados de este análisis de sensibilidad.

```{r graficos_sensibilidad_cartera}
# Se transpone el dataframe de personas hombres
df_personas <- as.data.frame(t(esp_per_hsen))
...
# Se transpone el dataframe de personas mujeres
df_personas <- as.data.frame(t(esp_per_msen))
...
# Dataframe de comparación de personas por sexo
df_comparacion <- esp_per_hsen - esp_per_msen
...
df_flujos <-
  data.frame(
    sexo = c("Hombre", "Mujer"),
    gastos = c(sum(esp_gast_hsen), sum(esp_gast_msen)),
    ingresos = c(sum(esp_ing_hsen), sum(esp_ing_msen))
  )
...
ggplot(df_flujos, aes(
  ...
  scale_fill_manual(
    name = "Tipo de Flujo",
    values = c(
      "gastos" = "paleturquoise2",
      "ingresos" = "palevioletred",
      "neto" = "purple2"
    ),
    labels = c("gastos" = "Gastos", "ingresos" = "Ingresos", "neto" = "Neto")
  ) +
...
Una de las variables que más se cree que afecta el análisis son las probabilidades de transición, debido a que se tomó un supuesto muy simplificado para estas, por lo cual, se reducirán en $10\%$ las probabilidades de estar sano y se repartirá dicho porcentaje en las probabilidades restantes de manera uniforme.

```{r modificacion_prob}
# Copia de las probabilidades para no afectar a las originales
prob_hombres_mod <- prob_hombres
prob_mujeres_mod <- prob_mujeres

# Se hace un ciclo para la modificación
for (i in 1:6) {
  # Se guardan los elementos a sumar
  modificacion_hombres <- ((prob_hombres_mod[[i]][, 1] * 0.05) / 5)
  modificacion_mujeres <- ((prob_mujeres_mod[[i]][, 1] * 0.05) / 5)
  
  # Se modifica la primer columna
  prob_hombres_mod[[i]][, 1] <- prob_hombres_mod[[i]][, 1] * 0.95
  prob_mujeres_mod[[i]][, 1] <- prob_mujeres_mod[[i]][, 1] * 0.95
  
  # Se modifican las demás columnas
  prob_hombres_mod[[i]][, 2:6] <- prob_hombres_mod[[i]][, 2:6] + modificacion_hombres
  prob_mujeres_mod[[i]][, 2:6] <- prob_mujeres_mod[[i]][, 2:6] + modificacion_mujeres
}
```

Ahora determinamos las primas comerciales usando estas probabilidades

```{r primas_mod}

# Vectores con las primas modifcadas
primas_modh <- c()
primas_modm <- c()
...
for (x in 18:64) {
  ...
  # Generamos la matriz con los kpx para ambos sexos
  kpx_h <- kpx(x, prob_hombres_mod)
  kpx_m <- kpx(x, prob_mujeres_mod)
  
  # Se determina la prima para un hombre de edad x
  
  # Seguro de vida completo
  Ax_05 <- Ax05(x, v_j, kpx_h, prob_hombres_mod)
  ...
  prima <- (S * Ax_05 + c1 * ax01 + c2 * ax02 + c3 * ax03 + c4 * ax04) / (0.9 *
                                                                           ax00 - 0.1)
  # Guardamos la prima para alguien de edad x en el vector de primas
  primas_modh <- c(primas_modh, prima)
  
  # Analogamente se determina la prima para una mujer de edad x
  
  # Seguro de vida completo
  Ax_05 <- Ax05(x, v_j, kpx_m, prob_mujeres_mod)
  ...
  prima <- (S * Ax_05 + c1 * ax01 + c2 * ax02 + c3 * ax03 + c4 * ax04) / (0.9 *
                                                                           ax00 - 0.1)
  # Guardamos la prima para alguien de edad x en el vector de primas
  primas_modm <- c(primas_modm, prima)
}
```

Se procede a obtener el modelo estocástico con las primas modificadas.

```{r modelo_probabilidades}
# Definimos la semilla para reproducibilidad
set.seed(2024)
...
# Ingresos por estado de hombres y mujeres
ingresos_h_mod <- data.frame(
  "0" = c(primas_modh, rep(0, 51)),
  "1" = rep(0, 98),
  "2" = rep(0, 98),
  "3" = rep(0, 98),
  "4" = rep(0, 98),
...
ingresos_m_mod <- data.frame(
  "0" = c(primas_modm, rep(0, 51)),
  "1" = rep(0, 98),
  "2" = rep(0, 98),
  "3" = rep(0, 98),
  "4" = rep(0, 98),
...
# Determinamos las matrices asociadas a las simulaciones determinadas, tanto para hombres y mujeres
simulaciones_h <- modelo_estoc(n_sim, prob_hombres_mod, hombres_cartera, ingresos_h_mod)
simulaciones_m <- modelo_estoc(n_sim, prob_mujeres_mod, mujeres_cartera, ingresos_m_mod)

# Matrices con las esperanzas por estado y para cada año, para cada sexo
esperanzas_personas_hprob <- Reduce("+", simulaciones_h[[1]]) / n_sim
esperanzas_personas_mprob <- Reduce("+", simulaciones_m[[1]]) / n_sim

# Esperanzas por estado de gastos
esperanzas_gastos_hprob <- Reduce("+", simulaciones_h[[2]]) / n_sim
esperanzas_gastos_mprob <- Reduce("+", simulaciones_m[[2]]) / n_sim

# Esperanzas por estado de ingresos
esperanzas_ingresos_hprob <- Reduce("+", simulaciones_h[[3]]) / n_sim
esperanzas_ingresos_mprob <- Reduce("+", simulaciones_m[[3]]) / n_sim
```

Graficamos los resultados de este análisis de sensibilidad.

```{r graficos_prob_cartera}
# Se transpone el dataframe de personas hombres
df_personas <- as.data.frame(t(esperanzas_personas_hprob))
...
# Se transpone el dataframe de personas mujeres
df_personas <- as.data.frame(t(esperanzas_personas_mprob))
...
# Dataframe de comparación de personas por sexo
df_comparacion <- esperanzas_personas_hprob - esperanzas_personas_mprob
...
df_flujos <-
  data.frame(
    sexo = c("Hombre", "Mujer"),
    gastos = c(sum(esperanzas_gastos_hprob), sum(esperanzas_gastos_mprob)),
    ingresos = c(sum(esperanzas_ingresos_hprob), sum(esperanzas_ingresos_mprob))
  )
...
# Sse juntan las primas para graficarlas
primas_brutas_mod <- data.frame(primas_modh, primas_modm)

# Nombre de filas y columnas
colnames(primas_brutas_mod) <- c("Hombre", "Mujer")
rownames(primas_brutas_mod) <- 18:64

# Gráficos de primas modificadas con gastos
ggplot(primas_brutas_mod[1:38, ], aes(x = 18:55)) +
  geom_line(aes(y = Hombre / 1000000, color = "H"), linewidth = 1) +
  geom_line(aes(y = Mujer / 1000000, color = "M"), linewidth = 1) +
  labs(x = "Edad", y = "Primas comerciales", color = "Leyenda") +
  scale_color_manual(
    name = "Sexo", 
    values = c("H" = "gold2", "M" = "royalblue"),
    labels = c("H" = "Hombre", "M" = "Mujer")
...
ggplot(primas_brutas_mod[39:47, ], aes(x = 56:64)) +
  geom_line(aes(y = Hombre / 1000000, color = "H"), linewidth = 1) +
  geom_line(aes(y = Mujer / 1000000, color = "M"), linewidth = 1) +
  labs(x = "Edad", y = "Primas comerciales", color = "Leyenda") +
  scale_color_manual(
