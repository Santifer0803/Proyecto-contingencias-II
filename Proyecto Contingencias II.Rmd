...
modelo_estoc <- function(simulaciones, tabla_prob) {
  ...
  for (simulacion in 1:simulaciones) {
    escenario <- matrix(NA, nrow = (115 - 18), ncol = 98)
    ...
    for (i in 1:47) {
      ...
      intervalos <- unname(c(0, cumsum(c(tabla_prob[[estado + 1]][i, ]))))
      
      if (length(intervalos) > 1) {
        for (j in 1:(length(intervalos) - 1)) {
          if (!(between(matriz_aleatoria[i, estado_comp + 1], intervalos[j], intervalos[j + 1]))) {
            estado <- estado + 1
